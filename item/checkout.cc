...
unique_ptr<checkout> checkout::make(json data, QPointF pos, item* parent, QColor color)
{
    Q_UNUSED(color);
    auto ret = unique_ptr <checkout> (new checkout(std::move(data), pos, parent));
...
checkout::checkout(json data, QPointF pos, item *parent)
...
//    const auto & serial_number = checkout_info_;
...
//        return;
//    }
    const auto serial_number = find_attribute("序号");
    if (serial_number != nullopt and !serial_number->empty())
    {
        ...
        const QRectF text_rect (center.x() - text_w / 2, center.y() - text_h / 2, text_w, text_h);
...
//    dlg.setIntValue(checkout_info_.toInt());
...
//    if (val == 0)
//    {
//        checkout_info_.clear();
//    }
//    else
//    {
//        checkout_info_ = QString::number(val);
//    }
